        ...
        public static async Task Main(string[] args)
        {
            ...
            using (var scope = host.Services.CreateScope())
            {
                ...
                try
                {
                    ...
                    await elasticPoRepos.ElasticSaveBulkAsync((await elasticPoRepos.GetPOForELIndexAsync(new PagingOption<PurchaseOrderSearchIndex>(0,0), new POSearchFilter(){Status = -99})).ResultList.ToArray(), "purchaseorders");
                    ...
                    await elasticGiRepos.ElasticSaveBulkAsync((await elasticGiRepos.GetGIForELIndexAsync(new PagingOption<GoodsIssueSearchIndex>(0,0), new GISearchFilter(){Status = -99})).ResultList.ToArray(), "goodsissueorders");
